...
from worlds.sc2lotv.Locations import SC2LOTVL_LOC_ID_OFFSET
...
class SC2Context(CommonContext):
    ...
    def on_package(self, cmd: str, args: dict):
        if cmd in {"Connected"}:
            ...
            self.final_mission = args["slot_data"].get("final_mission", 23)
    ...
    def build_location_to_mission_mapping(self):
        ...
        for loc in self.server_locations:
            mission_id, objective = divmod(loc - SC2LOTVL_LOC_ID_OFFSET, victory_modulo)
    ...
    def locations_for_mission(self, mission: str):
        ...
        for objective in objectives:
            yield SC2LOTVL_LOC_ID_OFFSET + mission_id * 100 + objective
...
maps_table = [
    "ap_paiur01", "ap_paiur02", "ap_paiur03", "ap_paiur04", "ap_paiur05", "ap_paiur06"
    "ap_pkorhal01", "ap_pkorhal02",
    "ap_pmoebius01",
    "ap_ppurifier01", "ap_ppurifier02", "ap_ppurifier03",
                ...
                if self.can_read_game:
                    if game_state & (1 << 1) and not self.mission_completed:
                        if self.mission_id != self.ctx.final_mission:
                            ...
                            await self.ctx.send_msgs(
                                [{"cmd": 'LocationChecks',
                                  "locations": [SC2LOTVL_LOC_ID_OFFSET + victory_modulo * self.mission_id]}])
                    ...
                    for x, completed in enumerate(self.boni):
                        if not completed and game_state & (1 << (x + 2)):
                            await self.ctx.send_msgs(
                                [{"cmd": 'LocationChecks',
                                  "locations": [SC2LOTVL_LOC_ID_OFFSET + victory_modulo * self.mission_id + x + 1]}])
                            self.boni[x] = True
...
def request_unfinished_missions(ctx: SC2Context):
    if ctx.mission_req_table:
        ...
        # Removing Salvation from location pool
    ...
    if len(ctx.mission_req_table[mission_name].required_world) >= 1:
        ...
        for req_mission in ctx.mission_req_table[mission_name].required_world:
            ...
            if not (ctx.mission_req_table[list(ctx.mission_req_table)[req_mission - 1]].id *
                    victory_modulo + SC2LOTVL_LOC_ID_OFFSET) in ctx.checked_locations:
...
def is_mod_installed_correctly() -> bool:
    ...
    # Check for maps.
    missing_maps = []
    for mapfile in lotvl_required_maps:
