...
GATEWAY_UNITS = {"Marine", "Medic", "Firebat", "Marauder", "Reaper", "Ghost", "Spectre"}
ROBOTICS_UNITS = {"Hellion", "Vulture", "Goliath", "Diamondback", "Siege Tank", "Thor", "Predator"}
STARGATE_UNITS = {"Medivac", "Wraith", "Viking", "Banshee", "Battlecruiser", "Hercules", "Science Vessel", "Raven"}

WHISPERS_REGIONS = {"Dark Whispers", "Ghosts in the Fog"}
...
def filter_missions(multiworld: MultiWorld, player: int) -> Dict[int, List[str]]:
    ...
    shuffle_prologue = get_option_value(multiworld, player, "shuffle_prologue")
    ...
    if not shuffle_prologue:
        excluded_missions = excluded_missions.union(WHISPERS_REGIONS)
    ...
    if not get_option_value(multiworld, player, 'shuffle_no_build'):
        ...
        move_mission("The Growing Shadow", MissionPools.EASY, MissionPools.STARTER)
        move_mission("The Spear of Adun", MissionPools.EASY, MissionPools.STARTER)
        move_mission("Amon's Reach", MissionPools.EASY, MissionPools.STARTER)
		move_mission("Brothers in Arms", MissionPools.EASY, MissionPools.STARTER)
        # Pushing extra Normal missions to Easy
        move_mission("Forbidden Weapon", MissionPools.MEDIUM, MissionPools.EASY)
        # Additional changes on Advanced Tactics
    if get_option_value(multiworld, player, "required_tactics") > 0:
        move_mission("Dark Whispers", MissionPools.MEDIUM, MissionPools.EASY)
        move_mission("Ghosts in the Fog", MissionPools.MEDIUM, MissionPools.EASY)
        move_mission("Unsealing the Past", MissionPools.MEDIUM, MissionPools.EASY)
...
class ValidInventory:
    ...
    def has_units_per_structure(self) -> bool:
        return len(GATEWAY_UNITS.intersection(self.logical_inventory)) > self.min_units_per_structure and \
            len(ROBOTICS_UNITS.intersection(self.logical_inventory)) > self.min_units_per_structure and \
            len(STARGATE_UNITS.intersection(self.logical_inventory)) > self.min_units_per_structure
    ...
    def _read_logic(self):
        ...
        #self._sc2lotv_has_air_anti_air = lambda world, player: SC2LotVLogic._sc2lotv_has_air_anti_air(self, world, player)
        #self._sc2lotv_has_competent_anti_air = lambda world, player: SC2LotVLogic._sc2lotv_has_competent_anti_air(self, world, player)
        self._sc2lotv_has_anti_air = lambda world, player: SC2LotVLogic._sc2lotv_has_anti_air(self, world, player)
        self._sc2lotv_defense_rating = lambda world, player: SC2LotVLogic._sc2lotv_defense_rating(self, world, player)
        #self._sc2lotv_has_competent_comp = lambda world, player: SC2LotVLogic._sc2lotv_has_competent_comp(self, world, player)
        #self._sc2lotv_has_train_killers = lambda world, player: SC2LotVLogic._sc2lotv_has_train_killers(self, world, player)
        #self._sc2lotv_able_to_rescue = lambda world, player: SC2LotVLogic._sc2lotv_able_to_rescue(self, world, player)
        #self._sc2lotv_beats_protoss_deathball = lambda world, player: SC2LotVLogic._sc2lotv_beats_protoss_deathball(self, world, player)
        #self._sc2lotv_survives_rip_field = lambda world, player: SC2LotVLogic._sc2lotv_survives_rip_field(self, world, player)
        #self._sc2lotv_has_protoss_common_units = lambda world, player: SC2LotVLogic._sc2lotv_has_protoss_common_units(self, world, player)
        #self._sc2lotv_has_protoss_medium_units = lambda world, player: SC2LotVLogic._sc2lotv_has_protoss_medium_units(self, world, player)
        #self._sc2lotv_has_mm_upgrade = lambda world, player: SC2LotVLogic._sc2lotv_has_mm_upgrade(self, world, player)
        #self._sc2lotv_final_mission_requirements = lambda world, player: SC2LotVLogic._sc2lotv_final_mission_requirements(self, world, player)
